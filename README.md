...
covering **every stock across every tab of the Excel report** — a
TradingView-style candlestick viewer with a **1D / 1W / 1M timeframe
toggle** and a **bar-by-bar replay control**, so you can watch a pole
and flag form instead of just reading the numbers.
...
- **Chart**: candlesticks + volume, a solid line tracing the actual
  pole move and a dashed channel marking the flag's high/low, dashed
  price lines for pivot/stop/target 1/target 2, and markers on the
  pole start, flag start, and breakout dates. The pole line and flag
  channel draw in progressively as replay reaches them, not upfront.
- **Replay**: play/pause, step one bar at a time, or drag the
  scrubber — starts a little before the pole so you see the setup form
  from a clean base. Keyboard shortcuts: `Space` play/pause, `←`/`→`
  step a bar, `↑`/`↓` jump to the previous/next symbol in the sidebar
  (built for flipping through a lot of charts quickly).
- **⊞ All Timeframes**: pops 1D + 1W + 1M into a separate window,
  stacked together with the same overlays, for comparing structure
  across timeframes without toggling back and forth.
