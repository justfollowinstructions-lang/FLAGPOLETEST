...
def macd(
    ...
    """
    Standard MACD (12/26/9 by default). Returns (macd_line, signal_line,
    histogram) — histogram = macd_line - signal_line.
    """
...
# ─── Universe-wide RS Rating (shared across scanner entry points) ─────────
# Kept here (rather than duplicated per entry point) so every scanner
# that imports this module computes RS Ratings identically, from one
# place — currently used by main_flag_pole.py.

def compute_universe_rs_ratings(symbols: list[str]) -> tuple[dict, dict]:
    """
    Returns (rs_ratings, rs_trends) — {symbol: RS Rating 1-99} and
    {symbol: 'Improving'/'Declining'/'Flat'/'Unknown'} — computed
    cross-sectionally across the given symbol list.
    """
    from config import MIN_DAILY_BARS, RS_TREND_LOOKBACK_WEEKS
    from downloader import load_daily

    raw_returns: dict[str, float] = {}
    raw_returns_4w_ago: dict[str, float] = {}
    ...
    for sym in symbols:
        ...
        if daily is None or len(daily) < MIN_DAILY_BARS:
            continue
        close = daily["Close"]
        val = raw_weighted_return(close)
        if val is not None:
            raw_returns[sym] = val

        lookback_bars = RS_TREND_LOOKBACK_WEEKS * 5
        if len(close) > lookback_bars:
            val_4w = raw_weighted_return(close.iloc[:-lookback_bars])
            if val_4w is not None:
                raw_returns_4w_ago[sym] = val_4w
