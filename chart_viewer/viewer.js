...
let viewMode = "chart";   // "chart" | "grid" | "analysis"
...
function buildSymbolItem(s) {
  ...
  const ratingChip = s.rating
    ? `<span class="rating-chip ${ratingBadgeClass(s.rating)}" style="margin-top:2px;">${esc(s.rating.label)}</span>`
    : "";
  el.innerHTML = `
    <div>
...
function selectSymbol(idx) {
  ...
  if (viewMode === "grid") {
    ...
    renderAnalysisPanel(s);
  } else {
    loadTimeframe(currentTF);
  }
}

function renderRatingBadge(s) {
  const el = document.getElementById("hdr-rating");
  if (!s.rating) { el.innerHTML = ""; return; }
  el.innerHTML = `<span class="rating-chip rating-${s.rating.color}" title="${esc(s.rating.reason)}">${esc(s.rating.label)}</span>`;
}

function ratingBadgeClass(rating) {
  return rating ? `rating-${rating.color}` : "rating-grey";
}
...
function buildMarkers(s) {
  ...
  const add = (date, position, color, shape, text) => {
    const snapped = snapToBarTime(currentBars, date);
    if (snapped) markers.push({ time: snapped, position, color, shape, text });
  };
...
function updatePatternOverlay(s) {
  if (s.pole_start_date && s.pole_end_date) {
    const startTime = snapToBarTime(currentBars, s.pole_start_date);
    const endTime = snapToBarTime(currentBars, s.pole_end_date);
    ...
    if (startTime && endTime && startTime !== endTime) {
      ...
      poleLineSeries.setData([]);   // pole collapses into a single bar at this zoom level — nothing meaningful to draw
    }
  } else {
    poleLineSeries.setData([]);
  }
  ...
  if (s.flag_start_date && s.flag_end_date && s.flag_high != null && s.flag_low != null) {
    const startTime = snapToBarTime(currentBars, s.flag_start_date);
    const endTime = snapToBarTime(currentBars, s.flag_end_date);
    if (startTime && endTime && startTime !== endTime) {
      flagUpperSeries.setData([{ time: startTime, value: s.flag_high }, { time: endTime, value: s.flag_high }]);
      flagLowerSeries.setData([{ time: startTime, value: s.flag_low }, { time: endTime, value: s.flag_low }]);
    } else {
...
// Snaps a daily-granularity date onto the TIME LABEL of the nearest
// actual bar in `bars` (which may be weekly/monthly-resampled).
// Critical fix: lightweight-charts, in business-day/date-string mode,
// builds its horizontal time axis from the union of every distinct
// time value across ALL series on a chart. A pole/flag point using a
// raw daily date that doesn't exactly match any weekly/monthly bar's
// own label (e.g. a Monday when weekly bars are labeled by that
// week's Monday but the actual detection date was a Tuesday) injects
// a brand-new axis slot with no candle drawn there — visible as a
// phantom gap with a stray date tick and nothing else. Snapping to an
// existing bar's own time value guarantees every overlay point lands
// exactly on a real candle position instead.
function snapToBarTime(bars, dateStr) {
  if (!dateStr || bars.length === 0) return null;
  let best = bars[0];
  for (const b of bars) {
    if (b.time <= dateStr) best = b; else break;
...
function applySettingsFromPanel() {
  ...
  if (currentSymbolIdx >= 0 && viewMode === "chart") updateMaSeriesData();
}
...
function setViewMode(mode) {
  // mode is "chart" | "grid" | "analysis"
  const prevMode = viewMode;
  viewMode = mode;

  document.getElementById("single-view").style.display = mode === "chart" ? "flex" : "none";
  document.getElementById("grid-view").classList.toggle("visible", mode === "grid");
  document.getElementById("analysis-panel").classList.toggle("visible", mode === "analysis");

  document.getElementById("btn-grid-view").classList.toggle("active", mode === "grid");
  document.getElementById("btn-toggle-analysis").classList.toggle("active", mode === "analysis");

  const tfLocked = mode !== "chart";
  document.getElementById("tf-toggle").querySelectorAll("button[data-tf]").forEach(b => {
    b.style.opacity = tfLocked ? "0.4" : "1";
    b.style.pointerEvents = tfLocked ? "none" : "auto";
  });

  if (prevMode === "grid" && mode !== "grid") destroyGridCharts();
  ...
  if (mode === "grid") {
    ...
    renderAnalysisPanel(s);
  } else if (mode === "chart" && prevMode !== "chart") {
    loadTimeframe(currentTF);
  }
}
...
function toggleAnalysisPanel() {
  setViewMode(viewMode === "analysis" ? "chart" : "analysis");
}

// ─────────────────────────────────────────────────────────────────────────
// Full Analysis panel — renders the rating/scanner_reasons/why_buy/
// explanation objects computed server-side in chart_export.py
// (see explain_flag_pole.py). Pure rendering, no computation here.
// ─────────────────────────────────────────────────────────────────────────
function renderAnalysisPanel(s) {
  renderReasonRows(s.scanner_reasons || []);
  renderExplanation(s.explanation);
  renderWhyBuy(s.why_buy);
  renderSellRules(s.sell_notes);
}

function renderReasonRows(reasons) {
  const container = document.getElementById("ap-reasons-rows");
  if (!reasons.length) {
    container.innerHTML = `<p style="color:var(--muted);">No detection rules recorded for this signal.</p>`;
    return;
  }
  const statusIcon = { pass: "✔", fail: "✘", warn: "!" };
  container.innerHTML = reasons.map(r => `
    <div class="reason-row">
      <div class="reason-status status-${r.status}">${statusIcon[r.status] || "?"}</div>
      <div>
        <div class="reason-rule">${esc(r.rule)}</div>
        <div class="reason-detail">${esc(r.detail)}</div>
      </div>
    </div>
  `).join("");
}
...
function renderExplanation(explanation) {
  ...
  document.getElementById("ap-why-detected").textContent = explanation.why_detected || "";
  document.getElementById("ap-strengths").innerHTML =
    (explanation.strengths || []).map(x => `<li>${esc(x)}</li>`).join("");
  document.getElementById("ap-weaknesses").innerHTML =
    (explanation.weaknesses || []).map(x => `<li>${esc(x)}</li>`).join("");
  document.getElementById("ap-institutional").textContent = explanation.institutional_note || "";
  document.getElementById("ap-conclusion").textContent = explanation.conclusion || "";
}

function renderWhyBuy(whyBuy) {
  const banner = document.getElementById("ap-recommend-banner");
  const paras = document.getElementById("ap-whybuy-paragraphs");
  if (!whyBuy) { banner.innerHTML = ""; paras.innerHTML = ""; return; }

  banner.innerHTML = whyBuy.recommend
    ? `<div class="recommend-banner recommend-yes">✔ Recommended by the scanner's own criteria</div>`
    : `<div class="recommend-banner recommend-no">✘ Not currently recommended</div>`;
  paras.innerHTML = (whyBuy.paragraphs || []).map(p => `<p>${esc(p)}</p>`).join("");
}

function renderSellRules(sellNotes) {
  const container = document.getElementById("ap-sellrules");
  if (!sellNotes) {
    container.innerHTML = `<p style="color:var(--muted);">No sell rules recorded.</p>`;
    return;
  }
  const parts = sellNotes.split("||").map(p => p.trim()).filter(Boolean);
  container.innerHTML = parts.map(p => `<p>${esc(p)}</p>`).join("");
}
...
function renderGridView(s) {
  ...
  ["1D", "1W", "1M"].forEach(tf => {
    ...
    const markers = [];
    const addMarker = (date, position, color, shape, text) => {
      const snapped = snapToBarTime(tfData.bars, date);
      if (snapped) markers.push({ time: snapped, position, color, shape, text });
    };
    addMarker(s.pole_start_date, "belowBar", "#58a6ff", "arrowUp", "Pole");
    addMarker(s.flag_start_date, "aboveBar", "#d29922", "circle", "Flag");
    addMarker(s.breakout_date, "belowBar", "#3fb950", "arrowUp", "Breakout");
    candles.setMarkers(markers);

    if (s.pole_start_date && s.pole_end_date) {
      const startTime = snapToBarTime(tfData.bars, s.pole_start_date);
      const endTime = snapToBarTime(tfData.bars, s.pole_end_date);
      ...
      if (startTime && endTime && startTime !== endTime) {
        ...
        poleLine.setData([{ time: startTime, value: startPrice }, { time: endTime, value: endPrice }]);
      }
    }
    if (s.flag_start_date && s.flag_end_date && s.flag_high != null && s.flag_low != null) {
      const startTime = snapToBarTime(tfData.bars, s.flag_start_date);
      const endTime = snapToBarTime(tfData.bars, s.flag_end_date);
      if (startTime && endTime && startTime !== endTime) {
        ...
        upper.setData([{ time: startTime, value: s.flag_high }, { time: endTime, value: s.flag_high }]);
        lower.setData([{ time: startTime, value: s.flag_low }, { time: endTime, value: s.flag_low }]);
      }
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("#tf-toggle button[data-tf]").forEach(b =>
    b.addEventListener("click", () => { if (currentSymbolIdx >= 0 && viewMode === "chart") loadTimeframe(b.dataset.tf); }));
  ...
  document.getElementById("btn-toggle-analysis").addEventListener("click", toggleAnalysisPanel);
