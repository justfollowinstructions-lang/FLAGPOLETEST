...
def test_export_produces_well_formed_html(temp_db, charts_dir, monkeypatch):
    ...
    assert len(sym["timeframes"]["1D"]["bars"]) > 0

    bar = sym["timeframes"]["1D"]["bars"][0]
...
def test_bars_respect_lookback_limits(temp_db, charts_dir, monkeypatch):
    ...
    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    assert len(sym["timeframes"]["1D"]["bars"]) <= cfg.CHART_BARS_LOOKBACK_DAILY
    assert len(sym["timeframes"]["1W"]["bars"]) <= cfg.CHART_BARS_LOOKBACK_WEEKLY
    assert len(sym["timeframes"]["1M"]["bars"]) <= cfg.CHART_BARS_LOOKBACK_MONTHLY
...
def test_indicator_keys_present_for_every_timeframe(temp_db, charts_dir, monkeypatch):
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=250)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    expected_keys = {"sma5", "sma10", "sma20", "sma30", "sma50",
                      "rsi14", "macd", "macd_signal", "macd_hist"}
    for tf in ("1D", "1W", "1M"):
        assert expected_keys == set(sym["timeframes"][tf]["indicators"].keys())


def test_moving_average_is_correct_from_the_first_visible_bar(temp_db, charts_dir, monkeypatch):
    """The whole point of computing indicators on full history before
    trimming: a 50DMA should already have a valid value at (or very
    near) the first bar of ITS OWN trimmed window, not need 50 more
    bars to 'warm up' inside that window. Bars and indicators now use
    deliberately different lookback windows (bars = effectively full
    history, indicators = a bounded recent window) — needs enough
    synthetic history beyond the indicator window (252 daily bars) to
    leave room for the 50-bar warm-up ahead of it."""
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=400)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    sma50 = sym["timeframes"]["1D"]["indicators"]["sma50"]

    assert len(sma50) > 0
    # sma50's own series should start within a couple of bars of ITS
    # OWN first entry — i.e. it isn't itself missing ~50 leading
    # points that a full recompute-in-window approach would have left
    # blank. We check this indirectly: sma50's point count should be
    # close to the configured indicator lookback, not ~50 short of it.
    assert len(sma50) >= cfg.CHART_INDICATOR_LOOKBACK_DAILY - 2


def test_bars_get_far_more_history_than_indicators(temp_db, charts_dir, monkeypatch):
    """Candles are meant to show effectively full listing history;
    indicator lines are deliberately trimmed to a much shorter recent
    window (see config.py's CHART_BARS_LOOKBACK_* vs
    CHART_INDICATOR_LOOKBACK_* comments) — this is what keeps the file
    size sane while still showing full price history."""
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=400)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    n_bars = len(sym["timeframes"]["1D"]["bars"])
    n_sma20 = len(sym["timeframes"]["1D"]["indicators"]["sma20"])

    assert n_bars > cfg.CHART_INDICATOR_LOOKBACK_DAILY   # far more candles than indicator points
    assert n_sma20 <= cfg.CHART_INDICATOR_LOOKBACK_DAILY
    bar_dates = [b["time"] for b in sym["timeframes"]["1D"]["bars"]]
    sma20_dates = [p["time"] for p in sym["timeframes"]["1D"]["indicators"]["sma20"]]
    # Candles reach further back in time than the indicator series does.
    assert bar_dates[0] < sma20_dates[0]


def test_rsi_values_are_within_0_100(temp_db, charts_dir, monkeypatch):
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=250)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    rsi_points = data["symbols"][0]["timeframes"]["1D"]["indicators"]["rsi14"]
    assert len(rsi_points) > 0
    assert all(0.0 <= p["value"] <= 100.0 for p in rsi_points)


def test_macd_histogram_equals_macd_minus_signal(temp_db, charts_dir, monkeypatch):
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=250)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    ind = data["symbols"][0]["timeframes"]["1D"]["indicators"]
    macd_by_time = {p["time"]: p["value"] for p in ind["macd"]}
    signal_by_time = {p["time"]: p["value"] for p in ind["macd_signal"]}
    hist_by_time = {p["time"]: p["value"] for p in ind["macd_hist"]}

    common_dates = set(macd_by_time) & set(signal_by_time) & set(hist_by_time)
    assert len(common_dates) > 0
    for t in common_dates:
        assert hist_by_time[t] == pytest.approx(macd_by_time[t] - signal_by_time[t], abs=0.02)


def test_short_history_symbol_gets_sparse_or_empty_long_period_ma(temp_db, charts_dir, monkeypatch):
    """A symbol with less than 50 bars of total history can't have a
    valid 50DMA anywhere — should come back empty, not crash or emit
    garbage values."""
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=60)  # just above MIN_DAILY_BARS_FLAG_POLE
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    sma50 = data["symbols"][0]["timeframes"]["1D"]["indicators"]["sma50"]
    sma5 = data["symbols"][0]["timeframes"]["1D"]["indicators"]["sma5"]
    assert len(sma5) > 0          # short MA should still have values
    assert len(sma50) <= len(sma5)  # long MA can't have more valid points than a shorter one
