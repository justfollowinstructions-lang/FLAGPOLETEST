...
def test_bars_respect_lookback_limits(temp_db, charts_dir, monkeypatch):
    ...
    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    assert len(sym["timeframes"]["1D"]["bars"]) <= cfg.CHART_LOOKBACK_DAILY_BARS
    assert len(sym["timeframes"]["1W"]["bars"]) <= cfg.CHART_LOOKBACK_WEEKLY_BARS
    assert len(sym["timeframes"]["1M"]["bars"]) <= cfg.CHART_LOOKBACK_MONTHLY_BARS
...
def test_moving_average_is_correct_from_the_first_visible_bar(temp_db, charts_dir, monkeypatch):
    ...
    near) the first visible bar, not need 50 more bars to 'warm up'
    inside the visible window. Needs enough synthetic history beyond
    the lookback window itself (252 daily bars) to leave room for the
    50-bar warm-up ahead of it."""
    _seed_signal(monkeypatch, symbol="AAA.NS", breakout=True, base_n=400)
    _run_export(monkeypatch)

    data, _ = _extract_data_blob(charts_dir)
    sym = data["symbols"][0]
    bars = sym["timeframes"]["1D"]["bars"]
    sma50 = sym["timeframes"]["1D"]["indicators"]["sma50"]

    assert len(sma50) > 0
    first_bar_date = bars[0]["time"]
    first_sma_date = sma50[0]["time"]
    # sma50's first point should be at or within a few bars of the
    # chart's first visible bar — not 50 bars later.
    bar_dates = [b["time"] for b in bars]
    assert first_sma_date in bar_dates
    assert bar_dates.index(first_sma_date) <= 2
