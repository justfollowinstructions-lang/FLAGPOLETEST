...
def resample_weekly(daily: pd.DataFrame) -> pd.DataFrame:
    """
    Weekly OHLCV via resampling (week closes on Friday). Deliberately
    uses pandas' default closed='right', label='right' for 'W-FRI' —
    bins are (previous Friday, this Friday], labeled with the Friday
    date. An earlier version explicitly overrode this to
    ...
    a broken single-day stub. Holidays/weekends need no special
    handling — resample naturally aggregates whatever trading days
    exist within each calendar week.
    """
    return (
