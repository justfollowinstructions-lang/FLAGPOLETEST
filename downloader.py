...
def resample_weekly(daily: pd.DataFrame) -> pd.DataFrame:
    """
    Weekly OHLCV via resampling (week closes on Friday, correctly
    grouping Mon-Fri together — see the bucketing fix noted below).

    Labeled with the MONDAY of each week, not the Friday. Confirmed
    against a real TradingView weekly chart: consecutive weekly bars
    were dated 27 Jul / 20 Jul / 13 Jul 2026 — all Mondays, exactly 7
    days apart. Bucketing uses pandas' correct default for 'W-FRI'
    (closed='right', label='right', bins are (previous Friday, this
    Friday]), which labels natively with the Friday date; the index is
    then shifted back 4 days to relabel with that week's Monday
    instead, without touching which days ended up in which bucket.

    An earlier version explicitly overrode the resample to
    closed='left', label='left', which shifted each week's Friday bar
    into the FOLLOWING week's bucket and left the most recent week as
    a broken single-day stub — fixed separately from the label
    convention here. Holidays/weekends need no special handling —
    resample naturally aggregates whatever trading days exist within
    each calendar week.
    """
    weekly = (
