...
def _build_symbol_payload(row: pd.Series, tabs: list[str]) -> Optional[dict]:
    ...
    timeframes = {
        "1D": _build_timeframe_payload(daily, cfg.CHART_LOOKBACK_DAILY_BARS),
        "1W": _build_timeframe_payload(resample_weekly(daily), cfg.CHART_LOOKBACK_WEEKLY_BARS),
        "1M": _build_timeframe_payload(resample_monthly(daily), cfg.CHART_LOOKBACK_MONTHLY_BARS),
    }
...
def _build_timeframe_payload(df: pd.DataFrame, lookback_bars: int) -> dict:
    """
    Computes indicators on the FULL history in df, then trims the
    aligned output to the last `lookback_bars` — critical ordering.
    Computing a 50-period SMA only on an already-trimmed window would
    leave the first 49 visible bars without a valid value; computing
    it on the full series first means every visible bar's SMA is
    correct from its own true preceding history, even history that
    isn't itself shown on the chart.
    """
    if df is None or df.empty:
    ...
    for period in MA_PERIODS:
        indicators[f"sma{period}"] = _series_to_points(sma_fn(close, period), lookback_bars)

    indicators["rsi14"] = _series_to_points(rsi_fn(close, 14), lookback_bars)

    macd_line, signal_line, hist = macd_fn(close)
    indicators["macd"] = _series_to_points(macd_line, lookback_bars)
    indicators["macd_signal"] = _series_to_points(signal_line, lookback_bars)
    indicators["macd_hist"] = _series_to_points(hist, lookback_bars)

    return {
        "bars": _bars_to_json(df, lookback_bars),
