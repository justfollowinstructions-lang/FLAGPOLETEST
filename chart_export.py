...
def _build_symbol_payload(row: pd.Series, tabs: list[str]) -> Optional[dict]:
    ...
    timeframes = {
        "1D": _bars_to_json(daily, cfg.CHART_LOOKBACK_DAILY_BARS),
        "1W": _bars_to_json(resample_weekly(daily), cfg.CHART_LOOKBACK_WEEKLY_BARS),
        "1M": _bars_to_json(resample_monthly(daily), cfg.CHART_LOOKBACK_MONTHLY_BARS),
    }
    if not timeframes["1D"]:
        return None
