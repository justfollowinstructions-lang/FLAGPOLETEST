...
# Candles/volume: effectively "full history since listing" — these
# caps are only a safety ceiling against pathological cases, not a
# real trim for any actual NSE stock (even the oldest listings with
# data back to 2000 are well under these numbers). This is what was
# explicitly asked for: every candle since a company's listing date.
CHART_BARS_LOOKBACK_DAILY   = 10000   # ~40 years of trading days
CHART_BARS_LOOKBACK_WEEKLY  = 2500    # ~48 years
CHART_BARS_LOOKBACK_MONTHLY = 600     # ~50 years

# Indicator lines (MA/RSI/MACD) are trimmed to a much shorter, genuinely
# useful recent window on purpose. A 20-year-old 50DMA line contributes
# real file weight (an extra numeric series per bar, for every one of
# the ~10,000 daily bars now shipped) without being meaningfully
# readable at the zoom level anyone actually looks at it — nobody is
# comparing a 2004 candle against a 2004 moving-average value on the
# same continuous chart as today's price action.
CHART_INDICATOR_LOOKBACK_DAILY   = 252   # ~1 year
CHART_INDICATOR_LOOKBACK_WEEKLY  = 260   # ~5 years
CHART_INDICATOR_LOOKBACK_MONTHLY = 96    # ~8 years
